...
class Setup():
    ...
    def format_prefix_noveg(self):
        return f'{self.extent}_{self.use}_{self.heat_wave_thres}_{self.hot_and_dry_thres}'
...
class Filter(Setup):
    ...
    def apply(self, da, season = None):
        ...
        """
        da = mask_low_evi_seasonal(mask_crop(self.fid, 
                                             mask_water(self.fid, 
                                                        mask_impervious(self.fid, da, self.impervious_threshold, self.extent, 'both'), 
                                                        self.extent, 'both'), 
                                            self.extent, 'both'), self.fid, self.name, self.extent, season)
        return da
...
class Events(Setup):
    ...
    def get_spatial_avg(self):
        ...
        with xr.open_dataset(os.path.join(path_intrim, 'Daymet', self.extent, 'spi_' + str(self.fid) + '.nc')) as hr:
            if self.use != 'yyz':
        ...
        tmax_clim = tmax[(tmax['time'].to_index().year >= 2003) & (tmax['time'].to_index().year <= 2016), :, :].groupby('time.month').mean('time')
        ...
        for mon in range(12):
            is_heat_wave[mon::12] = identify_high_extremes(tmax.values[mon::12], self.heat_wave_thres)

            temp1 = identify_high_extremes(tmax.values[mon::12], self.hot_and_dry_thres)
            temp2 = identify_low_extremes (spi.values[mon::12], 100 - self.hot_and_dry_thres)
            is_hot_and_dry[mon::12] = temp1 & temp2

        # Attach time vector to day 1 of the month
        tvec = pd.DatetimeIndex([date(t.year, t.month, 1) for t in tmax['time'].to_index()])
        ...
        for i in range(len(heat_wave_start)):
            heat_wave_intensity[i] = tmax.values[(tvec >= heat_wave_start[i]) & (tvec <= heat_wave_end[i])].mean()
            heat_wave_duration[i]  = relativedelta(heat_wave_end[i], heat_wave_start[i]).months + 1
        ...
        for i in range(len(hot_and_dry_start)):
            hot_intensity[i] = tmax.values[(tvec >= hot_and_dry_start[i]) & (tvec <= hot_and_dry_end[i])].mean()
            dry_intensity[i] = spi.values [(tvec >= hot_and_dry_start[i]) & (tvec <= hot_and_dry_end[i])].mean()
            hot_and_dry_duration[i]  = relativedelta(hot_and_dry_end[i], hot_and_dry_start[i]).months + 1

        # Save all relevant info in dataframe
        self.heat_wave   = pd.DataFrame({'fid': self.fid, 
                                         'start': heat_wave_start,
                                         'end': heat_wave_end, 
                                         'intensity': heat_wave_intensity,
                                         'duration': heat_wave_duration}).set_index(['fid','start','end'])
        self.hot_and_dry = pd.DataFrame({'fid': self.fid,
                                         'start':hot_and_dry_start,
                                         'end': hot_and_dry_end,
                                         'hot_intensity': hot_intensity,
                                         'dry_intensity': dry_intensity,
                                         'duration': hot_and_dry_duration}).set_index(['fid','start','end'])
    ...
    def get_aux_spatial_avg(self, varname, extreme_events):
        ...
        if varname == 'spi':
            ...
            if self.use == 'daymet':
                ...
                # Remove Feb 29
                var = var[(var['time'].to_index().month != 2) | (var['time'].to_index().day != 29), :, :]
            elif self.use == 'yyz': 
        ...
        if varname in ['tmax', 'tmin', 'vpd']:
            var_clim = var[(var['time'].to_index().year >= 2003) & (var['time'].to_index().year <= 2016), :, :].groupby('time.month').mean()
            var = var.groupby('time.month') - var_clim.mean(dim = ['row','col'])

        # Attach time vector to day 1 of the month
        tvec = pd.DatetimeIndex([date(t.year, t.month, 1) for t in var['time'].to_index()])

        urban_mask = get_mask(self.fid, 'core' , True, self.extent)
        rural_mask = get_mask(self.fid, 'rural', True, self.extent)

        var_series = pd.DataFrame({'urban': var.where(urban_mask).mean(dim = ['row', 'col']).values,
                                   'rural': var.where(rural_mask).mean(dim = ['row', 'col']).values,
                                   'total': var.mean(dim = ['row', 'col']).values},
                                  index = tvec)

        var = pd.DataFrame(np.nan,
                           index = pd.MultiIndex.from_tuples([list(i) + [j] for i in extreme_events.index for j in ['urban','rural','total']], names = ['fid','start','end','location']),
        ...
        for i, ind in enumerate(extreme_events.index):
            ...
            post_event = var_series.loc[(ind[2] + relativedelta(months = 1)):(ind[2] + relativedelta(months = 3)), :].mean(axis = 0)

            of_season  = var_series.loc[var_series.index.month.isin(season_to_month[month_to_season[ind[2].month]]), :].mean(axis = 0)
    ...
    def get_aux_pixel(self, varname, extreme_events):
        ...
        var = pd.DataFrame(np.nan,
                           index = pd.MultiIndex.from_tuples([list(i) + list(j) for i in extreme_events.index for j in var_df.columns], names = ['fid','start','end','row','col']),
                           columns = ['in_event', 'post_event', 'of_season'])

        for i, ind in enumerate(extreme_events.index):
            in_event   = var_df.loc[ind[1]:ind[2], :].mean(axis = 0)
            post_event = var_df.loc[(ind[2] + relativedelta(months = 1)):(ind[2] + relativedelta(months = 3)), :].mean(axis = 0)

            # ensure consistency between the time period of the datasets for the seasonal mean
            of_season  = var_df.loc[var_df.index.month.isin(season_to_month[month_to_season[ind[2].month]]) & \
                                    (var_df.index.year >= 2003) & (var_df.index.year <= 2016), :].mean(axis = 0)
...
class Veg(Setup):
    ...
    def get_spatial_avg(self, extreme_events):
        ...
        veg0 = veg0.groupby('time.month') - veg0.groupby('time.month').mean('time') + veg0.mean('time')
        mean_mean = veg0.mean('time')
        std_mean  = veg0.std('time')
        std_mean  = std_mean.where(std_mean > 0.)
        ...
        urban_mask = get_mask(self.fid, 'core', True, self.extent )
        rural_mask = get_mask(self.fid, 'rural', True, self.extent)
        tvec = pd.DatetimeIndex([date(t.year, t.month, 1) for t in veg0['time'].to_index()])
        veg  = pd.DataFrame({'urban': veg0.where(urban_mask).mean(dim = ['row', 'col']).values,
                             'rural': veg0.where(rural_mask).mean(dim = ['row', 'col']).values},
                            index = tvec)
        veg_mean_mean  = {'urban': float(mean_mean.where(urban_mask).mean()),
                          'rural': float(mean_mean.where(rural_mask).mean(dim = ['row', 'col']))}
        veg_std_mean   = {'urban': float(std_mean.where(urban_mask).mean()),
        ...
        self.veg_response = pd.DataFrame(np.nan,
                                         index = pd.MultiIndex.from_tuples([list(i) + [j] for i in extreme_events.index for j in ['urban','rural']], names = ['fid','start','end','location']),
                                         columns = [f'Month {i}' for i in range(*self.bracket)] + ['pre_veg_norm', 'Resistance', 'Resilience', 'Recovery'])
        ...
        for i, ind in enumerate(extreme_events.index):
            ...
            end   = min(ind[1] + relativedelta(months = self.bracket[-1] - 1), veg.index[-1])
            ...
            if i < (len(extreme_events.index) - 1):
                start_next = min(extreme_events.index[i+1][1] - relativedelta(months = 1), end)
                if end > start_next:
                    veg_temp = veg_temp.loc[:start_next, :]
                end = start_next

            days = [f'Month {i}' for i in range(int(relativedelta(start, ind[1]).years) * 12 + int(relativedelta(start, ind[1]).months),
                                                int(relativedelta(end  , ind[1]).years) * 12 + int(relativedelta(end  , ind[1]).months) + 1)]
            self.veg_response.loc[ind, days] = veg_temp.T.loc[['urban','rural'], :].values

            pre_event  = veg_temp.loc[mirror:(ind[1] - relativedelta(months = 1)), :].mean(axis = 0)

            # the sign * the maximum magnitude
            in_event   = veg_temp.loc[ind[1]:ind[2], :].apply(lambda x: x[np.argmax(np.abs(x))], axis = 0)
            ...
            post_event = veg_temp.loc[(ind[2] + relativedelta(months = 1)):(ind[2] + relativedelta(months = 3)), :].mean(axis = 0)
            ...
            for loc in ['urban', 'rural']:
                ...
                self.veg_response.loc[ind + (loc,), 'Resistance'] = 2 * (in_event[loc] - pre_event[loc]) / (pre_event[loc] + abs(in_event[loc] - pre_event[loc]))
                self.veg_response.loc[ind + (loc,), 'Resilience'] = 2 * (post_event[loc] - in_event[loc]) / (abs(post_event[loc] - in_event[loc]) + abs(pre_event[loc] - in_event[loc])) # larger = speedier recovery
                self.veg_response.loc[ind + (loc,), 'Recovery'] = 2 * (post_event[loc] - pre_event[loc]) / (pre_event[loc] + abs(pre_event[loc] - post_event[loc]))

                # Normalize the pre-event vegetation during the mirrored period
                self.veg_response.loc[ind + (loc,), 'pre_veg_norm'] = (pre_event[loc] - veg_mean_mean[loc]) / veg_std_mean[loc]
    ...
    def get_luc_avg(self, extreme_events):
        ...
        for ln in modis_luc_short_list:
            ...
            veg_mean_mean.loc[ln] = mean_mean.where(mask).mean(dim = ['row', 'col']).values
        ...
        # or if there is not any vegetation
        self.veg_response = pd.DataFrame(np.nan,
                                         index = pd.MultiIndex.from_tuples([list(i) + [j] for i in extreme_events.index for j in modis_luc_short_list], names = ['fid','start','end','location']),
                                         columns = [f'Month {i}' for i in range(*self.bracket)] + ['pre_veg_norm', 'Resistance', 'Resilience', 'Recovery'])
        ...
        for i, ind in enumerate(extreme_events.index):
            ...
            days = [f'Month {i}' for i in range(int(relativedelta(start, ind[1]).years) * 12 + int(relativedelta(start, ind[1]).months),
                                                int(relativedelta(end  , ind[1]).years) * 12 + int(relativedelta(end  , ind[1]).months) + 1)]
            self.veg_response.loc[ind, days] = veg_temp.T.loc[modis_luc_short_list, :].values
            ...
            for loc in modis_luc_short_list:
                ...
                self.veg_response.loc[ind + (loc,), 'Resistance'] = 2 * (in_event[loc] - pre_event[loc]) / (pre_event[loc] + abs(in_event[loc] - pre_event[loc]))
                self.veg_response.loc[ind + (loc,), 'Resilience'] = 2 * (post_event[loc] - in_event[loc]) / (abs(post_event[loc] - in_event[loc]) + abs(pre_event[loc] - in_event[loc])) # larger = speedier recovery
                self.veg_response.loc[ind + (loc,), 'Recovery']   = 2 * (post_event[loc] - pre_event[loc]) / (pre_event[loc] + abs(post_event[loc] - pre_event[loc]))
    ...
    def get_pixel(self, extreme_events):
        ...
        std_mean     = std_mean.where(std_mean > 0.)
        veg = (veg.groupby('time.month') - mean_monthly).groupby('time.month') / std_monthly * std_mean + mean_mean
        veg = da_to_df(veg)
        ...
        self.veg_response = pd.DataFrame(np.nan,
                                         index = pd.MultiIndex.from_tuples([list(i) + list(j) for i in extreme_events.index for j in veg.columns], names = ['fid','start','end','row','col']),
                                         columns = [f'Month {i}' for i in range(*self.bracket)]+ ['pre_veg_norm', 'Resistance', 'Resilience', 'Recovery'])
        ...
        for i, ind in enumerate(extreme_events.index):
            ...
            self.veg_response.loc[ind, days] = veg_temp.T.loc[self.veg_response.loc[ind,:].index, :].values

            # Use the inverse of Isbell et al. 2015, in order to have better stability (prevent the resistance from soaring into infinity)
            resistance = 2 * (in_event - pre_event) / (pre_event + abs(in_event - pre_event))
            resilience = 2 * (post_event - in_event) / (abs(post_event - in_event) + abs(pre_event - in_event)) # larger = speedier recovery
            recovery   = 2 * (post_event - pre_event) / (pre_event + abs(post_event - pre_event))

            self.veg_response.loc[ind, 'Resistance'] = resistance.loc[self.veg_response.loc[ind,:].index].values
            self.veg_response.loc[ind, 'Resilience'] = resilience.loc[self.veg_response.loc[ind,:].index].values
            self.veg_response.loc[ind, 'Recovery']   = recovery.loc  [self.veg_response.loc[ind,:].index].values

            # Normalize the pre-event vegetation during the mirrored period
            self.veg_response.loc[ind, 'pre_veg_norm'] = ((pre_event - veg_mean_mean) / veg_std_mean).loc[self.veg_response.loc[ind,:].index].values
...
class BackgroundClim(Setup):
    ...
    def calc(self):
        if self.use == 'daymet':
            tmax = read_daymet(self.fid, 'tmax', self.extent).resample({'time': '1MS'}).mean()
            # tmax = tmax.loc[tmax['time'].to_index().year >= 2000, :, :]

            tmin = read_daymet(self.fid, 'tmin', self.extent).resample({'time': '1MS'}).mean()
            # tmin = tmin.loc[tmin['time'].to_index().year >= 2000, :, :]

            prcp = read_daymet(self.fid, 'prcp', self.extent).resample({'time': '1MS'}).mean()
            # prcp = prcp.loc[prcp['time'].to_index().year >= 2000, :, :]

            with xr.open_dataset(os.path.join(path_intrim, 'Daymet', self.extent, f'spi_{self.fid}.nc')) as hr:
                spi = hr['spi'].copy(deep = True)
                # spi = spi.loc[spi['time'].to_index().year >= 2000, :, :].load()

            vpd = read_daymet(self.fid, 'vpd', self.extent).resample({'time': '1MS'}).mean()
        ...
        if self.use == 'daymet':
            ...
            prcp_urban = pd.Series(prcp_urban.values, index = prcp_urban['time'].to_index())

            prcp_rural = prcp.where(mask_rural).mean(dim = ['row', 'col'])
            prcp_rural = pd.Series(prcp_rural.values, index = prcp_rural['time'].to_index())
            ...
            data = pd.DataFrame({('prcp', 'all'): prcp_all, ('prcp', 'urban'): prcp_urban, ('prcp', 'rural'): prcp_rural,
                                 ('spi', 'all'): spi_all, ('spi', 'urban'): spi_urban, ('spi', 'rural'): spi_rural,
                                 ('vpd', 'all'): spi_all, ('vpd', 'urban'): spi_urban, ('vpd', 'rural'): spi_rural,
                                 ('tmax', 'all'): tmax_all, ('tmax', 'urban'): tmax_urban, ('tmax', 'rural'): tmax_rural, 
                                 ('tmin', 'all'): tmin_all, ('tmin', 'urban'): tmin_urban, ('tmin', 'rural'): tmin_rural})
        else:
            data = pd.DataFrame({('tmax', 'all'): tmax_all, ('tmax', 'urban'): tmax_urban, ('tmax', 'rural'): tmax_rural, 
                                 ('tmin', 'all'): tmin_all, ('tmin', 'urban'): tmin_urban, ('tmin', 'rural'): tmin_rural})
...
class LUC(Setup):
    ...
    def get_spatial_avg(self, extreme_events):
        impervious = mask_crop(self.fid, read_impervious(self.fid, 'core', self.extent), self.extent, 'urban')
        impervious_urban_frac = pd.Series(impervious.mean(dim = ['row','col']).values, index = impervious['year'])
        impervious_urban_size = pd.Series(impervious.sum(dim = ['row','col']).values, index = impervious['year'])

        impervious = mask_crop(self.fid, read_impervious(self.fid, 'rural', self.extent), self.extent, 'rural')
        impervious_rural_frac = pd.Series(impervious.mean(dim = ['row','col']).values, index = impervious['year'])
        impervious_rural_size = pd.Series(impervious.sum(dim = ['row','col']).values, index = impervious['year'])

        luc_urban = mask_crop(self.fid, read_nlcd(self.fid, 'core', self.extent), self.extent, 'urban')
        luc_urban = agg_nlcd(luc_urban).mean(dim = ['row','col'])

        luc_rural = mask_crop(self.fid, read_nlcd(self.fid, 'rural', self.extent), self.extent, 'rural')
        luc_rural = agg_nlcd(luc_rural).mean(dim = ['row','col'])

        luc = pd.DataFrame(np.concatenate([luc_urban.values, luc_rural.values], axis = 0), 
                           index = pd.MultiIndex.from_product([['urban','rural'], luc_urban['year'].values]), 
        ...
        for year in impervious['year'].values:
            ...
            data.loc[filt & (data.index.get_level_values(3) == 'urban'), 'impervious_frac'] = impervious_urban_frac.loc[year]
            data.loc[filt & (data.index.get_level_values(3) == 'urban'), 'impervious_size'] = impervious_urban_size.loc[year]
            data.loc[filt & (data.index.get_level_values(3) == 'rural'), 'impervious_frac'] = impervious_rural_frac.loc[year]
            data.loc[filt & (data.index.get_level_values(3) == 'rural'), 'impervious_size'] = impervious_rural_size.loc[year]
            for loc in ['urban', 'rural']:
                data.loc[filt & (data.index.get_level_values(3) == loc), luc.columns] = luc.loc[(loc, year), :].values

        return data

    def get_pixel(self, extreme_events):
        impervious = mask_crop(self.fid, read_impervious(self.fid, 'both', self.extent), self.extent, 'both')
        impervious = da_to_df(impervious, impervious['year'].to_index())

        luc        = mask_crop(self.fid, read_nlcd(self.fid, 'both', self.extent), self.extent, 'both')
        luc        =  agg_nlcd(luc)

        data = pd.DataFrame(np.nan,
                            index = pd.MultiIndex.from_tuples([list(i) + list(j) for i in extreme_events.index for j in impervious.columns], names = ['fid','start','end','row','col']),
                            columns = ['impervious_frac'] + list(luc['band'].values))
        for year in luc['year'].values:
            filt = data.index.get_level_values(2).year == year
            data.loc[filt, 'impervious_frac'] = impervious.loc[year, pd.MultiIndex.from_tuples(zip(data.index.get_level_values(3), data.index.get_level_values(4)))[filt]].values
            luc_temp = da_to_df(luc.loc[year, :, :, :], luc['band'].to_index())
            data.loc[filt, luc_temp.index] = luc_temp.loc[:, pd.MultiIndex.from_tuples(zip(data.index.get_level_values(3), data.index.get_level_values(4)))[filt]].values.T
        return data
...
class Elevation(Setup):
    ...
    def get_spatial_avg(self):
        ...
        return pd.Series({'urban': elev_urban, 'rural': elev_rural, 'diff': elev_urban - elev_rural})
...
class SeasonalAvgTemperaturePixel():
    ...
    def calc(self):
        ...
        """
        h = pd.HDFStore(os.path.join(path_out, 'clim', f'seasonal_average_{self.extent}_{self.heat_wave_thres}_{self.hot_and_dry_thres}.h5'), mode = 'w')
        for use in ['daymet', 'topowx', 'yyz']:
            for fid in tqdm(range(85)):
                ...
                tmax = tmax[tmax['time'].to_index().year >= 2001, :, :].resample({'time': 'Q-NOV'}).mean().groupby('time.month').mean()
                tmin = tmin[tmin['time'].to_index().year >= 2001, :, :].resample({'time': 'Q-NOV'}).mean().groupby('time.month').mean()
    ...
    def calc_mean(self):
        seasonal_average = pd.DataFrame(np.nan, index = range(85), columns = pd.MultiIndex.from_product([['urban', 'rural'], ['tmax', 'tmin'], ['DJF', 'MAM', 'JJA', 'SON'], ['daymet', 'topowx', 'yyz']]))

        h = pd.HDFStore(os.path.join(path_out, 'clim', f'seasonal_average_{self.extent}_{self.heat_wave_thres}_{self.hot_and_dry_thres}.h5'), mode = 'r')
        for use in ['daymet', 'topowx', 'yyz']:
            temp = add_urlabel_all(h.select(use), self.extent).groupby(['location', 'season', 'fid']).mean()
            ...
            for fid, varname in it.product(range(85), ['tmax', 'tmin']):
                for season in ['DJF', 'MAM', 'JJA', 'SON']:
                    for location in ['urban', 'rural']:
                        seasonal_average.loc[fid, (location, varname, season, use)] = temp.loc[(location, season, fid), varname]
...
class OptimalTPixel():
    ...
    def calc(self):
        ...
        def _find_optima(var_sr1, var_sr2, var_sr3, evi_sr, tvec):
            if len(var_sr1) == 0:
                return np.nan
            elif np.isnan(evi_sr[0]) | np.isnan(var_sr1[0]) | np.isnan(var_sr2[0]) | np.isnan(var_sr3[0]):
                return np.nan
            else:
                ...
                evi_mask = evi_mask.groupby(evi_mask.index.year).apply(lambda ts: ts >= 0.2 * (ts.max() - ts.min()))
                ...
                for i in range(len(edges) - 1):
                    ...
                    for sr in [var_sr1, var_sr2, var_sr3]:
                        pool = np.concatenate([pool, evi_sr[(sr >= edges[i]) & (sr < edges[i+1])]])
                    if len(pool) > 0:
        ...
        hsave = pd.HDFStore(os.path.join(path_out, 'veg', f'{self.prefix}_{self.extent}_{self.name}_optimalT.h5'), mode = 'w')
        ...
        for fid in tqdm(range(85)):
            ...
            tmax['topowx'] = tmax['topowx'][(tmax['topowx']['time'].to_index().month != 2) | (tmax['topowx']['time'].to_index().day != 29), :, :]
            tmin['topowx'] = tmin['topowx'][(tmin['topowx']['time'].to_index().month != 2) | (tmin['topowx']['time'].to_index().day != 29), :, :]
            ...
            for varname, var in zip(['tmax', 'tmin'], [tmax, tmin]):
                ...
                optima = xr.apply_ufunc(_find_optima, var1_temp, var2_temp, var3_temp, evi_temp,
                                        input_core_dims = [['time'], ['time'], ['time'], ['time']],
                                        output_core_dims = [[]], kwargs = {'tvec': temp},
    ...
    def calc_mean(self):
        optimum = pd.DataFrame(np.nan, index = range(85), columns = pd.MultiIndex.from_product([['tmax', 'tmin'], ['urban', 'rural']]))
        ...
        h.close()
        optimum.to_csv(os.path.join(path_out, 'veg', f'{self.prefix}_{self.extent}_{self.name}_optimalT.csv'))
...
class SensitivityWaterPixel():
    ...
    def calc(self):
        ...
        hsave = pd.HDFStore(os.path.join(path_out, 'veg', f'{self.prefix}_{self.extent}_{self.name}_sensitivity_water.h5'), mode = 'w')
        # optima_tmax, optima_tmin
        ...
        for fid in tqdm(range(85)):
            ...
            temp = evi['time'].to_index().intersection(spi['time'].to_index()).intersection(vpd['time'].to_index())
            ...
            for varname, var in zip(['spi', 'vpd'], [spi, vpd]):
                ...
                for season in ['DJF', 'MAM', 'JJA', 'SON']:
                    ...
                    corr, pval = xr.apply_ufunc(_corr_calc, var_, evi_, input_core_dims = [['time'], ['time']],
                ...
                corr_seasons = corr_seasons.reset_index().set_index(['fid', 'row', 'col'])

                pval_seasons = pd.DataFrame(pval_seasons)
                pval_seasons['fid'] = fid
                pval_seasons = pval_seasons.reset_index().set_index(['fid', 'row', 'col'])
    ...
    def calc_mean(self):
        seasonal_average = pd.DataFrame(np.nan, index = pd.MultiIndex.from_product([['DJF', 'MAM', 'JJA', 'SON'], range(85), ['urban', 'rural', 'total']], names = ['season', 'fid', 'location']),
        ...
        for use in ['corr_spi', 'corr_vpd']:
            ...
            for season in ['DJF', 'MAM', 'JJA', 'SON']:
                for location in ['urban', 'rural']:
                    seasonal_average.loc[(season, slice(None), location), use] = temp_1.loc[location, season].values
                seasonal_average.loc[(season, slice(None), 'total'), use] = temp_2.loc[season]
        self.seasonal_average = seasonal_average
        h.close()