...
def add_grid_coords(fid, df, extent):
    ...
    grid_list = pd.read_csv(
        os.path.join(path_intrim, "Daymet", "coords", extent + "_" + str(fid) + ".csv"),
        index_col=[0, 1],
    )
...
def add_urlabel_all(df, extent):
    ...
    for fid in np.unique(df["fid"]):
        grid_list = pd.read_csv(
            os.path.join(
                path_intrim, "Daymet", "coords", extent + "_" + str(fid) + ".csv"
            ),
            index_col=[0, 1],
        )
        df.loc[df["fid"] == fid, "location"] = [
            "urban" if grid_list.loc[(i, j), "core"] else "rural"
            for i, j in zip(
                df.loc[df["fid"] == fid, "row"], df.loc[df["fid"] == fid, "col"]
            )
        ]
...
def get_mask(
    fid, which=["both", "core", "rural"], clip=[True, False], opt=["tiff", "tiff_3x"]
):
    if which == "core":
    ...
    for yy in range(1980, 2021):
        if opt == "tiff":
            ds = rio.open(
                os.path.join(
                    ...
                    "Daymet",
                    opt,
                    "DAYMETv4_" + str(vv) + "_" + str(fid) + "_" + str(yy) + ".tif",
                )
            )
        else:
            ds = rio.open(
        ...
        if opt == "tiff_3x":
            ...
            bands = np.concatenate(
                [bands, np.full([bands.shape[0], 1, bands.shape[2]], np.nan)], axis=1
            )
            bands = np.concatenate(
                [bands, np.full([bands.shape[0], bands.shape[1], 1], np.nan)], axis=2
            )

        # tvec = [datetime.strptime(ii.split('_')[1], '%Y%m%d') for ii in ds.descriptions]
        tvec = [
            datetime(yy, 1, 1) + timedelta(days=ii)
...
def read_topowx(fid, opt=["tiff", "tiff_3x"]):
    flist = sorted(
        glob(
            os.path.join(
                path_data, "Meteorological", "TOPOWx", opt, f"reproj_fid{fid}_*.tif"
            )
        )
    )
    ...
    for i, filename in enumerate(flist):
        ...
        values[np.abs(values) >= 1e37] = np.nan
        f.close()

        dims = ["time", "row", "col"]
        coords = {
            "time": tvec,
            "row": range(values.shape[1]),
...
def read_yyz(fid, opt=["tiff", "tiff_3x"]):
    for vv in ["tmax", "tmin"]:
        flist = sorted(
            glob(
                os.path.join(
                    path_intrim, "Ta_USA_cities-selected", opt, f"{vv}_{fid}_*.tif"
                )
            )
        )
        ...
        for i, filename in enumerate(flist):
            ...
            if opt == "tiff_3x":
                ...
                values = np.concatenate(
                    [values, np.full([values.shape[0], 1, values.shape[2]], np.nan)],
                    axis=1,
                )
                values = np.concatenate(
                    [values, np.full([values.shape[0], values.shape[1], 1], np.nan)],
                    axis=2,
                )
...
def read_ssm(fid, mask=False):
    flist = sorted(
        glob(
            os.path.join(
                path_intrim, "gee", "SMAP10km", "SMAP10KM_ssm_" + str(fid) + "_*.tif"
            )
        )
    )
...
def read_pml(varname, fid, mask=False, extent="tiff"):
    flist = sorted(
        glob(
            os.path.join(
                path_intrim, "gee", "PMLv2", extent, varname + "_" + str(fid) + "_*.tif"
            )
        )
    )
    for i, fn in enumerate(flist):
...
def read_evi(fid, name, opt=["tiff", "tiff_3x"]):
    flist = sorted(
        glob(
            os.path.join(path_intrim, "vegetation", name, opt, f"reproj_fid{fid}_*.tif")
        )
    )
    for i, fn in enumerate(flist):
        ds = rio.open(fn)
        ...
        for yy in range(y1, y2 + 1):
            tvec += list(
                pd.date_range(str(yy) + "-01-01", str(yy) + "-12-31", freq=freq)
            )
...
def read_lai(fid, mask=False, opt=["tiff", "tiff_3x"]):
    ...
    for i, fn in enumerate(flist):
        ...
        for yy in range(y1, y2 + 1):
            tvec += list(
                pd.date_range(str(yy) + "-01-01", str(yy) + "-12-31", freq="8D")
            )
...
def read_et(fid, mask=False, opt=["tiff", "tiff_3x"]):
    flist = sorted(
        glob(
            os.path.join(
                path_intrim, "gee", "PMLv2", opt, "PMLv2_ET_" + str(fid) + "_*.tif"
            )
        )
    )
    for i, fn in enumerate(flist):
...
def read_rh(fid, opt=["tiff", "tiff_3x"]):
    ...
    """
    tmean = (
        read_daymet(fid, "tmin", True, opt) + read_daymet(fid, "tmax", True, opt).values
    ) / 2
...
def read_nlcd(fid, which=["core", "rural", "both"], opt=["tiff", "tiff_3x"]):
    f = rio.open(
        os.path.join(path_intrim, "gee_single", "NLCD", opt, f"NLCD_{fid:02d}.tif")
    )
    temp = f.read()
    f.close()

    dall = xr.DataArray(
        temp.reshape(8, -1, temp.shape[1], temp.shape[2]),
        dims=["year", "band", "row", "col"],
        coords={
...
def agg_nlcd(da):
    ...
    for i in coords["band"]:
        ...
        if len(da.shape) == 3:
            ...
            da_agg.loc[:, i, :, :] = (
                da.loc[:, detail, :, :].sum(axis=1, skipna=False).values
            )
        else:
...
def dominant_nlcd(da):
    ...
    if len(da.shape) == 4:
        ...
        to_mixed = (
            ((da.loc[:, 2, :, :] + 0.5 * da.loc[:, 3, :, :]) < 0.75 * tot_tree)
            & ((da.loc[:, 4, :, :] + 0.5 * da.loc[:, 3, :, :]) < 0.75 * tot_tree)
            & ((lcmax == 2) | (lcmax == 4))
        )
        lcmax = lcmax.where(~to_mixed, 3)
        lcmax = lcmax.where(
            np.isnan(lcmax.values[0, :, :]) | (da.max(dim="band") > 0.5), -1
        )
    elif len(da.shape) == 3:
        tot_tree = da.loc[2, :, :] + da.loc[3, :, :] + da.loc[4, :, :]
        to_mixed = (
...
def read_impervious(fid, which=["core", "rural", "both"], opt=["tiff", "tiff_3x"]):
    f = rio.open(
        os.path.join(
            path_intrim, "gee_single", "Impervious", opt, f"impervious_{fid:02d}.tif"
        )
    )
...
def read_elevation(fid, which=["core", "rural", "both"], opt=["tiff", "tiff_3x"]):
    f = rio.open(
        os.path.join(
            path_intrim, "gee_single", "DEM", opt, "DEM_{:02d}.tif".format(fid)
        )
    )
    ...
    elev = xr.DataArray(
        elev,
        dims=["row", "col"],
        coords={"row": range(elev.shape[0]), "col": range(elev.shape[1])},
    )
...
def subset_mask(fid, df, opt=["tiff", "tiff_3x"], which="both"):
    ...
    rowcol_list = pd.MultiIndex.from_tuples(
        list(zip(rowlist, collist)), names=["row", "col"]
    )
...
def normalize(ma_array):
    ...
    temp = (temp - n_mean) / np.where(n_std > 0.0, n_std, np.nan)
...
def olsTensor(Y, x):
    ...
    if len(x.shape) != 1:
        ...
        x = np.ma.array(
            np.broadcast_to(x.data.reshape(-1, 1, 1), Y.shape),
    ...
    xx = np.where(x.mask == False, x.data, 0.0)
    yy = np.where(Y.mask == False, Y.data, 0.0)
    ...
    beta = np.einsum(
        ...
        np.einsum(
            "ijk,ljk->ilk",
            np.linalg.pinv(
                np.einsum("ijk,ilk->jlk", xx, xx).transpose(2, 0, 1)
            ).transpose(1, 2, 0),
            xx,
        ),
        yy,
...
def da_to_sr(da):
    ...
    da = pd.Series(
        ...
        index=pd.MultiIndex.from_arrays(
            [row_list.reshape(-1), col_list.reshape(-1)], names=["row", "col"]
        ),
    )
    return da
...
def round_to_magnitude(vector, nsig):
    ...
    def _round_to_digits(a, magnitude):
        return np.around(a / np.power(10, magnitude), nsig - 1) * np.power(
            10, magnitude
        )
...
def create_reasonable_bins(vec, nbins):
    ...
    nsig = 2 + int(
        np.floor(
            np.log10(
                (np.max(vec_bins_abs) - np.min(vec_bins_abs)) / np.max(vec_bins_abs)
            )
        )
    )
...
def add_luc_label_all(df, extent):
    ...
    for fid in df["fid"].unique():
        df.loc[df["fid"] == fid, "luc"] = add_luc_label(
            fid, df.loc[df["fid"] == fid], extent
        )["luc"]
...
def orthogonal_regression(x, y, num_preds=1):
    ...
    if num_preds == 1:
        ...
        x_std = x.std(axis=1)
    mydata = odr.Data(x, y, wd=1.0 / np.power(x_std, 2), we=1.0 / np.power(y.std(), 2))
    myodr = odr.ODR(mydata, linear, beta0=[1.0 for n in range(num_preds + 1)])
    myoutput = myodr.run()
