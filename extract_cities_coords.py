...
def get_latlon(fid, extent):
    ...
    if extent == "tiff":
        ds = rio.open(
            os.path.join(path_intrim, "Daymet", extent, f"DAYMETv4_tmax_{fid}_{yy}.tif")
        )
    else:
        ds = rio.open(
            os.path.join(path_intrim, "Daymet", extent, f"tmax_{fid}_{yy}.tif")
        )

    row, col, x, y = np.array_split(
        np.array(
            [
                (
                    i,
...
for fid in range(85):
    ...
    coord.to_csv(
        os.path.join(path_intrim, "Daymet", "coords", f"{extent}_{fid}.csv"),
        index=False,
    )
    ...
    if fid in [28, 29]:
        ...
        fig.savefig(
            os.path.join(path_intrim, "Daymet", "coords", f"check_{extent}_{fid}.png")
        )
