...
if __name__ == "__main__":
    ...
    for fid in range(85):
        ...
        veg0 = mask_water(
            fid,
            mask_impervious(fid, read_evi(fid, name, extent), 0.8, extent, "both"),
            extent,
            "both",
        )
        ...
        h = rio.open(
            os.path.join(path_intrim, "Daymet", extent, f"tmax_{fid}_2001.tif")
        )
        profile = dict(h.profile)
        profile["width"] = seasonal_mean.shape[2]
        profile["height"] = seasonal_mean.shape[1]
        profile["count"] = (
            seasonal_mean.shape[0]
            + events_mean_in["daymet"].shape[0]
            + events_mean_in["topowx"].shape[0]
            + events_mean_in["yyz"].shape[0]
        )
        h.close()

        ds = rio.open(
            os.path.join(path_out, "veg", extent, f"{name}_{fid}_mask.tif"),
            "w",
            **profile,
        )
        ...
        for use in ["daymet", "topowx", "yyz"]:
            for j in range(events_mean_in[use].shape[0]):
                ...
                end_of_event = datetime.strftime(
                    events_mean_in[use]["time"].to_index()[j], "%Y%m%d"
                )
