...
setup = Setup('daymet')
data = pd.DataFrame(np.nan, index = pd.MultiIndex.from_product([['DJF', 'MAM', 'JJA', 'SON'], range(85)]),
                    columns = ['prcp', 'tmean'])
for fid in range(85):
  temp = pd.read_csv(os.path.join(path_out, 'clim', 'time_series', f'daymet_{fid}.csv'), index_col = 0, header = [0, 1], parse_dates = True)
  ...
  for i, season in enumerate(['DJF', 'MAM', 'JJA', 'SON']):
    ...
    data.loc[(season, fid), 'tmean'] = (temp.loc[i+1, ('tmax', 'all')] + temp.loc[i+1, ('tmin', 'all')]) / 2
data.to_csv(os.path.join(path_out, 'clim', f'clim_{setup.format_prefix_noveg_noevent()}.csv'))
...
for i, varname in enumerate(['tmean', 'prcp']):
  for j, season in enumerate(['DJF', 'MAM', 'JJA', 'SON']):
    ...
    cf = map.plot(inset_bar = False, ax = ax, map_args = dict(vmin = vmin, vmax = vmax, cmap = cmap_div()))
    ax.set_title(varname)
    plt.colorbar(cf, ax = ax, orientation = 'horizontal')
fig.savefig(os.path.join(path_out, 'clim', 'plots', f'clim_{setup.format_prefix_noveg_noevent()}.png'), dpi = 600., bbox_inches = 'tight')
plt.close(fig)
...
for use in ['daymet', 'topowx', 'yyz']:
  setup = Setup(use)
  with pd.HDFStore(os.path.join(path_out, 'luc', f'percity_per_pixel_{setup.format_prefix_noveg()}.h5'), mode = 'w') as h:
      with pd.HDFStore(os.path.join(path_out, 'extreme_events', f'percity_{setup.format_prefix_noveg()}.h5'), mode = 'r') as hx:
  ...
  with pd.HDFStore(os.path.join(path_out, 'luc', f'percity_per_pixel_{setup.format_prefix_noveg()}.h5'), mode = 'r') as h:
    for fid in range(85):
      df = h.select('heat_wave').loc[fid, :].reset_index().groupby(['row','col']).mean()

      fig, axes = plt.subplots(3, 3, figsize = (15, 15), subplot_kw = {'projection': crs_daymet})
      for i, varname in enumerate(['impervious_frac'] + sorted(modis_luc_agg_names.keys())):
        ax = axes.flat[i]

        if varname == 'impervious_frac':
          temp = df[[varname]] / 100. # convert to fraction for ease of plotting
          vmin = 0
          vmax = 0.8
        else:
          temp = df[[varname]]
        ...
        cf = ax.scatter(temp['x'].values, temp['y'].values, c = temp[varname].values, s = 10,
                        lw = 0., marker = 's', transform = crs_daymet, cmap = 'Reds',
                        norm = norm)
      ...
      cb = plt.colorbar(cf, cax = cax, shrink = 0.5, label = varname, orientation = 'horizontal')
      fig.savefig(os.path.join(path_out, 'luc', 'plots', f'percity_per_pixel_{setup.format_prefix_noveg()}_{fid}.png'), dpi = 600., bbox_inches = 'tight')
      plt.close(fig)
...
# ------------------------------------------------------------------------------------
setup = Setup('daymet')
with pd.HDFStore(os.path.join(path_out, 'elev', f'percity_per_pixel_{setup.extent}.h5'), mode = 'w') as h:
  for fid in tqdm(range(85)):
    u = Elevation(fid)
    data = u.get_pixel()
    h.append('elev', data)

with pd.HDFStore(os.path.join(path_out, 'elev', f'percity_per_pixel_{setup.extent}.h5'), mode = 'r') as h:
  for fid in range(85):
    ...
    norm = mpl.colors.SymLogNorm(linthresh = 100, vmin = temp['elev'].min(), vmax =temp['elev'].max())

    cf = ax.scatter(temp['x'].values, temp['y'].values, c = temp['elev'].values, s = 10,
                    lw = 0., marker = 's', transform = crs_daymet, cmap = 'copper_r',
                    norm = norm)
    add_core_boundary(ax, fid, 'k')
    ax.set_title('Elevation (m)')
    cax = fig.add_axes([0.1, 0.05, 0.8, 0.01])
    cb = plt.colorbar(cf, cax = cax, shrink = 0.5, orientation = 'horizontal')
    fig.savefig(os.path.join(path_out, 'elev', 'plots', f'percity_per_pixel_{setup.extent}_{fid}.png'), dpi = 600., bbox_inches = 'tight')
    plt.close(fig)
...
for use in ['daymet', 'topowx', 'yyz']:
  setup = Setup(use)
  with pd.HDFStore(os.path.join(path_out, 'extreme_events', f'percity_{setup.format_prefix_noveg()}.h5'),
                   mode = 'w') as h:
    for fid in tqdm(range(85)):
      m = Events(fid)
  ...
  with pd.HDFStore(os.path.join(path_out, 'extreme_events', f'percity_{setup.format_prefix_noveg()}.h5'), mode = 'r') as h:
    for ex in ['heat_wave', 'hot_and_dry']:
      df = h.select(ex)

      df['season'] = pd.DatetimeIndex(df.reset_index()['end']).month.map(month_to_season)

      temp = []
      for i in df.drop(['duration','season'], axis = 1).columns:
        temp.append(pd.DataFrame({'val': df[i], 'metric': i, 'season': df['season']}))

      new_df = pd.concat(temp + [pd.DataFrame({'val': df['duration'], 'season': df['season'],
                                              'metric': 'duration'})],
                        axis = 0).reset_index(drop = True)

      fgd = sns.displot(data = new_df, x = 'val', col = 'metric', row = 'season', kde = True)
      fgd.savefig(os.path.join(path_out, 'extreme_events', 'plots', f'percity_{setup.format_prefix_noveg()}_dist_of_{ex}_metrics.png'), dpi = 600.)
      ...
      for j, season in enumerate(['DJF','MAM','JJA','SON']):
        ...
        for fid in range(85):
          if fid in temp.index.get_level_values(0):
            interval += [relativedelta(a, b).years * 12 + relativedelta(a, b).months  \
                        for a,b in zip(pd.DatetimeIndex(temp.loc[fid,:].reset_index()['start'])[1:],
                                        pd.DatetimeIndex(temp.loc[fid,:].reset_index()['end'])[:-1])]
        ...
        if j == 0:
          ax.set_title(ex)
    fig.savefig(os.path.join(path_out, 'extreme_events', 'plots', f'percity_{setup.format_prefix_noveg()}_dist_of_intervals.png'), dpi = 600.)
    plt.close(fig)
  ...
  for j, season in enumerate(['DJF','MAM','JJA','SON']):
    ...
    cf = map.plot(inset_bar = False, ax = ax, point_scale = 0.4, map_args = dict(vmin = vmin, vmax = vmax, cmap = 'RdYlBu_r'))
    if j == 0:
    ...
    plt.colorbar(cf, cax = cax, orientation = 'horizontal')
  fig.savefig(os.path.join(path_out, 'extreme_events', 'plots', f'percity_{setup.format_prefix_noveg()}_events_map_{ex}.png'), dpi = 600., bbox_inches = 'tight')
  plt.close(fig)
  ...
  fig, axes = plt.subplots(4, 4, figsize = (17, 12), sharex = True, sharey = True,
                          subplot_kw = {'projection': ccrs.AlbersEqualArea(central_longitude = -100, central_latitude = 35)})
  fig.subplots_adjust(hspace = 0.01, wspace = 0.01)
  for j, season in enumerate(['DJF','MAM','JJA','SON']):
    temp = df.loc[df['season'] == season, :]
    ...
    vmax = 7.5
    map = MapOfColors('point', n_events)
    cf = map.plot(inset_bar = False, ax = ax, map_args = dict(vmin = vmin, vmax = vmax, cmap = 'RdYlBu_r'))
    if j == 0:
      ax.set_title('Number of events')
    ...
    vmax = 3
    map = MapOfColors('point', intensity)
    cf = map.plot(inset_bar = False, ax = ax, map_args = dict(vmin = vmin, vmax = vmax, cmap = 'RdYlBu'))
    if j == 0:
  ...
  fig, ax = plt.subplots(figsize = (4, 3), sharex = True, sharey = True,
                        subplot_kw = {'projection': ccrs.AlbersEqualArea(central_longitude = -100, central_latitude = 35)})
  # count the number of events that are intensity >= 2 degC, and duration >= 4 months.
  pct_long_and_intense_events = ((temp['intensity'] >= 2) & (temp['duration'] >= 4)).groupby(level = 0).mean() * 100.
  for fid in range(85):
  ...
  cf = map.plot(inset_bar = False, ax = ax, point_scale = 0.4, map_args = dict(vmin = 0, vmax = 60., cmap = 'viridis'))
  ax.text(-0.05, 0.5, season, rotation = 90, transform = ax.transAxes)
  cax = fig.add_axes([0.1, 0.1, 0.8, 0.01])
  plt.colorbar(cf, cax = cax, orientation = 'horizontal', label = 'Percentage long and intense events')
  fig.savefig(os.path.join(path_out, 'extreme_events', 'plots', f'percity_{setup.format_prefix_noveg()}_long_and_intense_events_map_{ex}.png'), dpi = 600., bbox_inches = 'tight')
  plt.close(fig)
...
for varname, use in it.product(['tmax', 'tmin', 'spi', 'vpd'], ['daymet', 'topowx', 'yyz']):
  setup = Setup(use)
  with pd.HDFStore(os.path.join(path_out, 'clim', f'percity_per_pixel_{varname}_{setup.format_prefix_noveg()}.h5'), mode = 'w') as h:
    with pd.HDFStore(os.path.join(path_out, 'extreme_events', f'percity_{setup.format_prefix_noveg()}.h5'), mode = 'r') as hx:
      for ex in ['heat_wave', 'hot_and_dry']:
        extreme_events = hx.select(ex)
        for fid in tqdm(range(85)):
...
for use, fid in it.product(['daymet', 'topowx', 'yyz'], range(85)):
  setup = Setup(use)
  fig, axes = plt.subplots(4, 4, figsize = (20, 20), subplot_kw = {'projection': crs_daymet})
  for i, varname in enumerate(['tmax', 'tmin', 'vpd', 'spi']):
    ...
    with pd.HDFStore(os.path.join(path_out, 'clim', f'percity_per_pixel_{varname}_{setup.format_prefix_noveg()}.h5'), mode = 'r') as h:
      norm = mpl.colors.Normalize(vmin = vmin, vmax = vmax)
      ...
      for j, ex in enumerate(['heat_wave', 'hot_and_dry']):
        for k, tt in enumerate(['in_event', 'post_event']):
          ...
          temp = h.select(ex).loc[fid, [tt]].reset_index().groupby(['row','col']).mean()
          temp = add_grid_coords(fid, temp, setup.extent)

          cf = ax.scatter(temp['x'].values, temp['y'].values, c = temp[tt].values, s = 10,
                          lw = 0., marker = 's', transform = crs_daymet, cmap = 'RdYlBu_r',
                          norm = norm)
          add_core_boundary(ax, fid, 'k')
          if i == 0:
            ax.set_title(f'{ex} {tt}')
          if (j == 0) & (k == 0):
            ax.text(-0.2, 0.5, f'{varname}', transform = ax.transAxes, rotation = 90)
          elif (j*2 + k) == 3:
            cax = fig.add_axes([0.05 + 0.24 * i, 0.05, 0.22, 0.01])
            cb = plt.colorbar(cf, cax = cax, shrink = 0.5, label = varname, orientation = 'horizontal')
  fig.savefig(os.path.join(path_out, 'clim', 'plots', f'percity_per_pixel_{setup.format_prefix_noveg()}_{fid}.png'), dpi = 600., bbox_inches = 'tight')
  plt.close(fig)
...
h = pd.HDFStore(os.path.join(path_out, 'veg', f'{s.prefix}_{s.extent}_{s.name}_sensitivity_water.h5'), mode = 'r')
for varname in ['vpd', 'spi']:
  corr = h.select(f'corr_{varname}').copy(deep = True)
  pval = h.select(f'pval_{varname}').copy(deep = True)

  corr = pd.DataFrame(np.where(pval <= 0.05, corr, 0.), index = corr.index, columns = corr.columns)
  ...
  corr_sig = pval.groupby(['location', 'fid']).mean() >= 0.5 # at least 50% significant

  def sig(gdf):
      pvalues = pd.Series(np.nan, ['DJF', 'MAM', 'JJA', 'SON'])
      for season in ['DJF', 'MAM', 'JJA', 'SON']:
          pvalues.loc[season] = \
              mannwhitneyu(gdf.loc[gdf['location'] == 'urban', season].values, 
                          gdf.loc[gdf['location'] == 'rural', season].values).pvalue
      return pvalues

  sig_diff = corr.groupby('fid').apply(sig)

  fig, axes = plt.subplots(4, 3, figsize = (15, 13), subplot_kw = {'projection': ccrs.AlbersEqualArea(central_longitude = -100, central_latitude = 35)})
  fig.subplots_adjust(hspace = 0.01, wspace = 0.01)
  for j, season in enumerate(['DJF', 'MAM', 'JJA', 'SON']):
      for i, location in enumerate(['urban', 'rural', 'urban-rural']):
          ...
          if i < 2:
              general = pd.DataFrame({'value': corr_mean.loc[location, season], 'sig': corr_sig.loc[location, season]})
              if varname == 'spi':
                  mag = {'cmap': 'RdBu', 'norm': BoundaryNorm(np.linspace(-.2, .2, 9), ncolors = 256, extend = 'both')}
              else:
                  mag = {'cmap': 'RdBu', 'norm': BoundaryNorm(np.linspace(-.2, .2, 9), ncolors = 256, extend = 'both')}
          else:
              general = pd.DataFrame({'value': corr_mean.loc['urban', season] - corr_mean.loc['rural', season], 
                                      'sig': sig_diff.loc[:, season] <= 0.05})
              mag = {'cmap': 'RdBu', 'norm': BoundaryNorm(np.linspace(-.12, .12, 7), ncolors = 256, extend = 'both')}
          ...
          if i == 0:
              ax.text(-0.1, 0.5, season, rotation = 90, verticalalignment = 'center', transform = ax.transAxes)
          if j == 0:
              if i < 2:
                  ax.set_title(location.capitalize())
              else:
                  ax.set_title('Urban $-$ Rural')
          ax.text(0.02, 0.93, lab[j*3 + i], fontweight = 'bold', transform = ax.transAxes)
  ...
  fig.savefig(os.path.join(path_out, 'veg', f'{s.prefix}_{s.extent}_{s.name}_sensitivity_{varname}.png'), dpi = 600., bbox_inches = 'tight')
  plt.close(fig)
...
for use in ['daymet', 'topowx', 'yyz']:
  setup = Setup(use)
  with pd.HDFStore(os.path.join(path_out, 'luc', f'percity_spatial_avg_{setup.format_prefix_noveg()}.h5'), mode = 'w') as h:
      with pd.HDFStore(os.path.join(path_out, 'extreme_events', f'percity_{setup.format_prefix_noveg()}.h5'), mode = 'r') as hx:
        for ex in ['heat_wave', 'hot_and_dry']:
          extreme_events = hx.select(ex)
          for fid in tqdm(range(85)):
  ...
  lab = 'abcdefghijkopqrstuvwxyz'
  fig, axes = plt.subplots(3, 1, figsize = (6.5, 7), subplot_kw = {'projection': ccrs.AlbersEqualArea(central_longitude = -100, central_latitude = 35)})
  ...
  for i, nn in enumerate(['urban', 'rural']):
      ...
      with pd.HDFStore(os.path.join(path_out, 'luc', f'percity_spatial_avg_{setup.format_prefix_noveg()}.h5'), mode = 'r') as h:
          ...
          if i == 1:
            ...
            legend = False
          map.plot(ax = ax, inset_bar = False, legend = legend, legend_args = {'ncol': 1, 'loc': 'right', 'bbox_to_anchor': (1.5, 0.5)})
  ...
  fig.savefig(os.path.join(path_out, 'luc', 'plots', f'percity_spatial_avg_luc_{setup.format_prefix_noveg()}.png'), dpi = 600., bbox_inches = 'tight')
  plt.close(fig)
...
data.index.name = 'fid'
data.to_csv(os.path.join(path_out, 'elev', f'urban_rural_difference_{setup.extent}.csv'))
...
fig, ax, cf = map.plot(inset_bar = False, map_args = dict(vmin = -220, vmax = 5, cmap = 'viridis'))
ax.set_title('Elevation difference (meters)')
plt.colorbar(cf, ax = ax, orientation = 'horizontal', shrink = 0.7, aspect = 50, pad = 0.05)
fig.savefig(os.path.join(path_out, 'elev', 'plots', f'urban_rural_difference_{setup.extent}.png'), dpi = 600., bbox_inches = 'tight')
plt.close(fig)
...
# ------------------------------------------------------------------------------------
for varname, use in it.product(['tmax', 'tmin', 'vpd', 'spi'], ['daymet', 'topowx', 'yyz']):
  if (varname == 'tmax') | ((varname == 'tmin') & (use == 'daymet')):
     continue

  setup = Setup(use)
  with pd.HDFStore(os.path.join(path_out, 'clim', f'percity_spatial_avg_{varname}_{setup.format_prefix_noveg()}.h5'), mode = 'w') as h:
    with pd.HDFStore(os.path.join(path_out, 'extreme_events', f'percity_{setup.format_prefix_noveg()}.h5'), mode = 'r') as hx:
      for ex in ['heat_wave', 'hot_and_dry']:
        extreme_events = hx.select(ex)
        for fid in tqdm(range(85)):
...
for use in ['daymet', 'topowx', 'yyz']:
  ...
  for i, varname in enumerate(['tmax', 'tmin', 'spi', 'vpd']):
    if varname in ['tmin', 'tmax']:
      ...
      raise 'Not implemented'
    with pd.HDFStore(os.path.join(path_out, 'clim', f'percity_spatial_avg_{varname}_{setup.format_prefix_noveg()}.h5'), mode = 'r') as h:
        for j, ex in enumerate(['heat_wave', 'hot_and_dry']):
          for k, tt in enumerate(['in_event', 'post_event']):
            ax = axes[i, j*2 + k]

            df = h.select(ex).reset_index().groupby(['fid','location']).mean()[tt].unstack()
            df = df['urban'] - df['rural']

            map = MapOfColors('point', df)
            cf = map.plot(inset_bar = True, ax = ax, map_args = dict(vmin = vmin, vmax = vmax, cmap = cmap_div()))

            if i == 0:
              ax.set_title(f'{ex} {tt}')
            if j == 0:
              ax.text(-0.1, 0.5, f'{varname}', transform = ax.transAxes, rotation = 90)
            plt.colorbar(cf, ax = ax, orientation = 'horizontal')
  fig.savefig(os.path.join(path_out, 'clim', 'plots', f'percity_spatial_avg_{setup.format_prefix()}.png'), dpi = 600., bbox_inches = 'tight')
  plt.close(fig)
...
# ------------------------------------------------------------------------------------
with pd.HDFStore(os.path.join(path_out, 'extreme_events', f'percity_{setup.format_prefix_noveg()}.h5'), mode = 'r') as hx:
  with pd.HDFStore(os.path.join(path_out, 'veg_response', f'percity_per_pixel_{setup.format_prefix()}.h5'), mode = 'w') as h:
    for ex in ['heat_wave', 'hot_and_dry']:
      extreme_events = hx.select(ex)
      for fid in tqdm(range(85)):
        v = Veg(fid)
...
with pd.HDFStore(os.path.join(path_out, 'veg_response', f'percity_per_pixel_{setup.format_prefix()}.h5'), mode = 'r') as h:
  df = h.select('heat_wave')[['Recovery', 'Resistance']]
  df['season'] = pd.DatetimeIndex(df.index.get_level_values(2)).month.map(month_to_season)
  df = df.reset_index()

  for fid in tqdm(range(85)):
    fig, axes = plt.subplots(4, 2, figsize = (10, 20), subplot_kw = {'projection': crs_daymet})
    for i, season in enumerate(['DJF', 'MAM', 'JJA', 'SON']):
      for j, metric in enumerate(['Resistance', 'Recovery']):
        ...
        temp = df.loc[(df['fid'] == fid) & (df['season'] == season), ['start', 'end', 'row', 'col'] + [metric]].groupby(['row', 'col']).mean()
        temp = add_grid_coords(fid, temp, setup.extent)

        cf = ax.scatter(temp['x'].values, temp['y'].values, c = temp[metric].values, s = 10,
                        lw = 0., marker = 's', transform = crs_daymet, cmap = 'RdYlBu',
                        norm = norm)
        add_core_boundary(ax, fid, 'k')
        if j == 0:
          ax.text(-0.2, 0.5, season, rotation = 90, transform = ax.transAxes)
        if i == 0:
          ...
          cb = plt.colorbar(cf, cax = cax, shrink = 0.5, label = metric, orientation = 'horizontal')

    fig.savefig(os.path.join(path_out, 'veg_response', 'plots', f'percity_per_pixel_{setup.format_prefix()}_metrics_{fid}.png'), dpi = 600., bbox_inches = 'tight')
    plt.close(fig)
...
s = OptimalTPixel()
s.calc_mean()
optimum = pd.read_csv(os.path.join(path_out, 'veg', f'{s.prefix}_{s.extent}_{s.name}_optimalT.csv'), index_col = 0, header = [0, 1])

gap_to_optimalT = pd.DataFrame(np.nan, index = seasonal_average.index, columns = seasonal_average.columns)
for varname, location, season, use in it.product(['tmax', 'tmin'], ['urban', 'rural'], ['DJF', 'MAM', 'JJA', 'SON'], ['daymet', 'topowx', 'yyz']):
  gap_to_optimalT.loc[:, (location, varname, season, use)] = \
    seasonal_average.loc[:, (location, varname, season, use)] - optimum.loc[:, (varname, location)].values
gap_to_optimalT.to_csv(os.path.join(path_out, 'veg', f'{s.prefix}_{s.extent}_{s.name}_gap_to_optimalT.csv'))
...
for varname in ['tmax', 'tmin']:
    ...
    for i, season in enumerate(['DJF', 'MAM', 'JJA', 'SON']):
        for j, location in enumerate(['urban', 'rural']):
            ...
            m = MapOfColors('point', seasonal_average.loc[:, (location, varname, season)].mean(axis = 1) - optimum.loc[:, (varname, location)])
            cf = m.plot(ax = ax, inset_bar = False, map_args = {'norm': BoundaryNorm(np.arange(-20, 20.1, 2.5), 256, extend = 'both'), 'cmap': 'RdBu'})
            if i == 0:
                ax.set_title('$\Delta$' + location.capitalize())
            ax.text(0.02, 0.9, lab[i * 3 + j], transform = ax.transAxes, weight = 'bold')

            if j == 0:
                ax.text(-0.1, 0.5, season, verticalalignment = 'center', rotation = 90, transform = ax.transAxes)
        ...
        ax = axes[i, 2]
        result = (seasonal_average.loc[:, ('urban', varname, season)].mean(axis = 1) - optimum.loc[:, (varname, 'urban')]) - \
                 (seasonal_average.loc[:, ('rural', varname, season)].mean(axis = 1) - optimum.loc[:, (varname, 'rural')])
        m = MapOfColors('point', result)
        cf = m.plot(ax = ax, inset_bar = True, map_args = {'norm': BoundaryNorm(np.arange(-2, 2.01, 0.5), 256, extend = 'both'), 'cmap': 'RdBu'}, bar_args = {'color': ['#313695', '#4575b4', '#a50026', '#d73027']})
        if i == 0:
        ...
        if season == 'JJA':
            ...
            diff = (seasonal_average.loc[:, ('urban', varname, season)].mean(axis = 1) - optimum.loc[:, (varname, 'urban')]) - \
                    (seasonal_average.loc[:, ('rural', varname, season)].mean(axis = 1) - optimum.loc[:, (varname, 'rural')])
    ...
    fig.savefig(os.path.join(path_out, 'veg', f'{s.prefix}_{s.extent}_{s.name}_{varname}_gap_to_optimalT.png'), dpi = 600., bbox_inches = 'tight')
    plt.close(fig)


lab = 'abcdefghijklmnopqrstuvwxyz'
fig, axes = plt.subplots(2, 3, figsize = (15, 8), subplot_kw = {'projection': ccrs.AlbersEqualArea(central_longitude = -100, central_latitude = 35)})
fig.subplots_adjust(hspace = 0.01, wspace = 0.01)
for i, varname in enumerate(['tmax', 'tmin']):
    for j, location in enumerate(['urban', 'rural']):
        ax = axes[i, j]
        m = MapOfColors('point', optimum.loc[:, (varname, location)])
        cf = m.plot(ax = ax, inset_bar = False, map_args = {'norm': BoundaryNorm(np.arange(7.5, 32.6, 2.5), 256, extend = 'both'), 'cmap': 'RdYlBu'})
        if i == 0:
            ax.set_title(location.capitalize())
        ax.text(0.02, 0.9, lab[i * 3 + j], transform = ax.transAxes, weight = 'bold')

        if j == 0:
            ax.text(-0.1, 0.5, f'{varname}' + '$_{optimum}$', verticalalignment = 'center', rotation = 90, transform = ax.transAxes)
    ...
    ax = axes[i, 2]
    m = MapOfColors('point', result)
    cf = m.plot(ax = ax, inset_bar = True, map_args = {'norm': BoundaryNorm(np.arange(-2, 2.1, 0.1), 256, extend = 'both'), 'cmap': 'RdBu'}, 
...
fig.savefig(os.path.join(path_out, 'veg', f'{s.prefix}_{s.extent}_{s.name}_optimalT.png'), dpi = 600., bbox_inches = 'tight')
plt.close(fig)
...
s.calc_mean()
seasonal_average = s.seasonal_average
seasonal_average.to_csv(os.path.join(path_out, 'veg', f'{s.prefix}_{s.extent}_{s.name}_sensWater.csv'))
...
for varname in ['spi', 'vpd']:
    ...
    for i, season in enumerate(['DJF', 'MAM', 'JJA', 'SON']):
        for j, location in enumerate(['urban', 'rural']):
            ax = axes[i, j]
            # (plot averages over the three meteorological data sets)
            m = MapOfColors('point', pd.Series(seasonal_average.loc[(season, slice(None), location), f'corr_{varname}'].values, index = range(85)))
            cf = m.plot(ax = ax, inset_bar = False, map_args = {'norm': BoundaryNorm(np.arange(-0.6, 0.6, 0.05), 256, extend = 'both'), 'cmap': 'RdBu'})
            if i == 0:
                ax.set_title('$\Delta$' + location.capitalize())
            ax.text(0.02, 0.9, lab[i * 3 + j], transform = ax.transAxes, weight = 'bold')
        ...
        ax = axes[i, 2]
        result = pd.Series(seasonal_average.loc[(season, slice(None), 'urban'), f'corr_{varname}'].values - \
                           seasonal_average.loc[(season, slice(None), 'rural'), f'corr_{varname}'].values, index = range(85))
        m = MapOfColors('point', result)
        cf = m.plot(ax = ax, inset_bar = True, map_args = {'norm': BoundaryNorm(np.arange(-0.6, 0.6, 0.05), 256, extend = 'both'), 'cmap': 'RdBu'}, bar_args = {'color': ['#313695', '#4575b4', '#a50026', '#d73027']})
        if i == 0:
            ax.set_title('$\Delta$(Urban \u2212 Rural)')
        ax.text(0.02, 0.9, lab[i * 3 + 2], transform = ax.transAxes, weight = 'bold')
        res = wilcoxon(result)
    ...
    fig.savefig(os.path.join(path_out, 'veg', f'{s.prefix}_{s.extent}_{s.name}_{varname}_sensitivity_water.png'), dpi = 600., bbox_inches = 'tight')
    plt.close(fig)
