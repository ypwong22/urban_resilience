...
for fid in range(85):
    ...
    for yr in range(2001, 2020, 5):
        outvrt = os.path.join(path_data, 'Vegetation', 'EVI', 'tempdir', extent, 'vsimem',
                              f'stacked_{fid}_{yr}.vrt')
        outtif = os.path.join(path_data, 'Vegetation', 'EVI', 'processed', extent,
                              f'reproj_fid{fid}_{yr}-{min(yr+4,2019)}.tif')
        tifs = sorted(glob(os.path.join(path_data, 'Vegetation', 'EVI', 'tempdir', extent,
                                        f'reproj_{fid}_{yr}*.tif'))) + \
               sorted(glob(os.path.join(path_data, 'Vegetation', 'EVI', 'tempdir', extent,
                                        f'reproj_{fid}_{yr+1}*.tif'))) + \
               sorted(glob(os.path.join(path_data, 'Vegetation', 'EVI', 'tempdir', extent,
                                        f'reproj_{fid}_{yr+2}*.tif'))) + \
               sorted(glob(os.path.join(path_data, 'Vegetation', 'EVI', 'tempdir', extent,
                                        f'reproj_{fid}_{yr+3}*.tif'))) + \
               sorted(glob(os.path.join(path_data, 'Vegetation', 'EVI', 'tempdir', extent,
                                        f'reproj_{fid}_{yr+4}*.tif')))
...
#
for fid in range(85):
    values = read_evi(fid, False, extent)
    ...
    fig.savefig(os.path.join(path_data, 'Vegetation', 'EVI', 'processed', extent,
