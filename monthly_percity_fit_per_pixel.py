...
class Regression(PixelData):
    def __init__(self, use, extreme, season, land_cover, city_list, target, which, n_estimators):
        super().__init__(use, extreme, season, land_cover, city_list)
        self.target = target # Resistance/Recovery
        self.which = which # Sign, Mag
        self.n_estimators = n_estimators
        self.reg = None
        self.norm = None
        self.run_suffix = f'{self.format_prefix()}_{self.extreme}_{self.season}_{self.land_cover}_{self.target}_{self.n_estimators}_{self.which}'
    ...
    def run_oob(self, alt_n = None):
        ...
        if self.which == 'Sign':
            self.reg = RandomForestClassifier(n_estimators = self.n_estimators, max_depth = X_train_norm.shape[1] - 1,
            ...
            self.reg = RandomForestRegressor(n_estimators = self.n_estimators, max_depth = X_train_norm.shape[1] - 1,
                                             n_jobs = mp.cpu_count() // 2, oob_score = True)
        self.reg = self.reg.fit(X_train_norm.values, y_train)
    ...
    def run(self, alt_n = None, refit = False):
        ...
        if refit:
            if self.which == 'Sign':
                self.reg = RandomForestClassifier(n_estimators = self.n_estimators, max_depth = X_train_norm.shape[1] - 1, n_jobs = mp.cpu_count() // 2, oob_score = True)
            else:
        ...
        if self.which == 'Sign':
            y_pred_train = self.reg.predict_proba(X_train_norm.values) # two classes
            y_pred_test  = self.reg.predict_proba(X_test_norm.values)

            precision_train, recall_train, f1score_train, _ = precision_recall_fscore_support(y_train, (y_pred_train[:, 1] >= 0.5).astype(int))
            bs_train = brier_score_loss(y_train, y_pred_train[:, 1]) # In effect, it is the mean squared error of the forecast

            precision_test, recall_test, f1score_test, _ = precision_recall_fscore_support(y_test, (y_pred_test[:, 1] >= 0.5).astype(int))
            bs_test = brier_score_loss(y_test, y_pred_test[:, 1])

            self.performance = pd.DataFrame({'precision': [precision_train[1], precision_test[1]],  'recall': [recall_train[1], recall_test[1]], 'f1score': [f1score_train[1], f1score_test[1]],  'brier': [bs_train, bs_test]}, index = ['test', 'train'])
        else:
        ...
        if refit:
            ...
            joblib.dump(self.reg, os.path.join(path_out, 'measures', 'regression_per_pixel', 'fit', f'{self.prefix}_{self.run_suffix}.sav'))
            self.norm.save(os.path.join(path_out, 'measures', 'regression_per_pixel', 'fit', f'{self.prefix}_norm_{self.run_suffix}.csv'))
    ...
    def shap(self):
        ...
        with pd.HDFStore(filename, mode = 'a') as hf:
            ...
            if self.which == 'Sign':
                # The two classes add up to zero
                sv = pd.DataFrame(shap_values[:, :, 1], index = X_test_norm.index, columns = X_test_norm.columns)
            else:
...
if __name__ == '__main__':
    ...
    use = 'REPLACE0'
    opt = 'REPLACE1'
    ex = 'REPLACE2' # 'heat_wave'
    season = 'REPLACE3' # 'JJA'
    land_cover = luc_list[REPLACE4] # 'Evergreen'
    target = 'REPLACE5' # 'Resistance'
    which = 'REPLACE6' # 'Sign', 'Mag'
    ...
    if opt == 'regression':
        r = Regression(use, ex, season, land_cover, luc_cities[land_cover], target, which, 1000)
    ...
    if opt == 'shap':
        ...
        start = time()
        r = Regression(use, ex, season, land_cover, luc_cities[land_cover], target, which, n_estimators)
        r.run(refit = True)
        end = time()
        print(f'The model took {(end - start) / 60} minutes.')
